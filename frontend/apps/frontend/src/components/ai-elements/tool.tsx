...
const getStatusBadge = (status: ToolUIPart["state"]) => {
  ...
  const icons = {
    ...
    "approval-requested": <ClockIcon className="size-4 text-orange-600" />,
...
const mapRenderResultTypeToState = (type: RenderOutputResult["type"]): ToolUIPart["state"] => {
...
export const ToolInput = ({ className, input, ...props }: ToolInputProps) => (
  <div className={cn("space-y-2 overflow-hidden p-4", className)} {...props}>
    ...
    <CodeBlock code={JSON.stringify(input, null, 2)} language="json">
      <CopyToClipboard text={JSON.stringify(input, null, 2)} />
    </CodeBlock>
  </div>
);
...
export const ToolOutput = ({ className, part, network, ...props }: ToolOutputProps) => {
  const renderResult =
    ...
    <div className={cn("space-y-3 p-4", className)} {...props}>
      ...
      >
        ...
        {renderResult.type === "success" ? (
          ...
          <CodeBlock code={JSON.stringify(renderResult.content, null, 2)} language="json">
            <CopyToClipboard text={JSON.stringify(renderResult.content, null, 2)} />
          </CodeBlock>
        ) : null}
      </div>
      {/* @ts-expect-error */}
      {part.output?._meta?.["x402.payment-response"] && (
        <>
          ...
          <div className="flex items-center gap-2">
            ...
            <CopyToClipboard
              // @ts-expect-error
              text={part.output._meta["x402.payment-response"].transaction}
            />
          </div>
        </>
      )}
...
function renderRawOutput({ output }: { output: ToolUIPart["output"] }): RenderOutputResult {
  ...
  return {
    type: "success",
    content: <Response>{parseResult.data.content.map((item) => item.text).join("")}</Response>,
  };
}
